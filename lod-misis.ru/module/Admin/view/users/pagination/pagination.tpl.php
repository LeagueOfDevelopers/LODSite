...
<ul class="pagination">
    ...
    <?php else: ?>
        <li><a href="/adminium/users?page=<?=$pagination['left']['value']?>"><?=$pagination['left']['view_symbol']?></a></li>
    <?php endif; ?>
    ...
    <?php foreach ($pagination['pagination'] as $entity): ?>
        ...
        <?php endif; ?>
        <?php if (!$entity['active']): ?>
            <li><a href="/adminium/users?page=<?=$entity['page']?>"><?=$entity['view_number']?></a></li>
        <?php endif; ?>
    <?php endforeach; ?>
    <?php if ($pagination['right']['disabled']): ?>
        <li class="disabled"><span><?=$pagination['right']['view_symbol']?></span></li>
    <?php else: ?>
        <li><a href="/adminium/users?page=<?=$pagination['right']['value']?>"><?=$pagination['right']['view_symbol']?></a></li>
    <?php endif; ?>
</ul>